...
class qmagClient(FacialRecognition):
    """
    qmag model class
    """
...
def load_model(
    ...
    """
    Construct qmag model, download its weights and load
    Returns:
        model (Model)
    """
    base_model = ResNet34()
    inputs = base_model.inputs[0]
    qmag_model = base_model.outputs[0]
    qmag_model = BatchNormalization(momentum=0.9, epsilon=2e-5)(qmag_model)
    qmag_model = Dropout(0.4)(qmag_model)
    qmag_model = Flatten()(qmag_model)
    qmag_model = Dense(512, activation=None, use_bias=True, kernel_initializer="glorot_normal")(
        qmag_model
    )
